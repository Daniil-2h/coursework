...
def home(request):
    ...
    # Общая выручка — сумма всех полученных платежей
    ...
    # Данные для аналитики (ежедневные продажи за последние 30 дней)
...
def customer_add(request):
    ...
    return render(request, 'core/customer_form.html', {'form': form, 'title': 'Добавление клиента'})
...
def customer_edit(request, pk):
    ...
    if form.is_valid():
        ...
        return redirect('customer_list')
    return render(request, 'core/customer_form.html', {'form': form, 'title': 'Редактирование клиента'})
...
def customer_detail(request, pk):
    ...
    total_paid = Payment.objects.filter(invoice__customer=customer).aggregate(Sum('amount_paid'))['amount_paid__sum'] or Decimal('0.00')
    balance = total_invoiced - total_paid
    
    # Рассчитываем остаток по каждому счёту
    for inv in invoices:
...
def customer_quick_add(request):
    try:
        ...
        if not name or not email:
            return JsonResponse({'status': 'error', 'message': 'Имя и Email обязательны для заполнения.'}, status=400)
            
        if Customer.objects.filter(email=email).exists():
            return JsonResponse({'status': 'error', 'message': 'Клиент с таким Email уже существует.'}, status=400)
...
def product_add(request):
    ...
    return render(request, 'core/product_form.html', {'form': form, 'title': 'Добавление товара'})
...
def product_edit(request, pk):
    ...
    if form.is_valid():
        form.save()
        messages.success(request, "Данные товара обновлены!")
        return redirect('product_list')
    return render(request, 'core/product_form.html', {'form': form, 'title': 'Редактирование товара'})

def product_delete(request, pk):
    product = get_object_or_404(Product, pk=pk)
    if request.method == 'POST':
        product.delete()
        messages.success(request, "Товар удалён из справочника.")
        return redirect('product_list')
    ...
    if request.method == 'POST':
        try:
            ...
            if not customer_id or not items_data:
                return JsonResponse({'status': 'error', 'message': 'Не выбран клиент или не добавлены товары.'}, status=400)
            ...
            for item in items_data:
                ...
                # Сохраняем позицию — она сама спишет остатки
            ...
            messages.success(request, f"Счёт {invoice.invoice_number} успешно создан!")
...
def invoice_list(request):
    ...
    # Добавляем сумму оплат и остаток по каждому счёту
    for inv in invoices:
        inv.paid_amount = Payment.objects.filter(invoice=inv).aggregate(Sum('amount_paid'))['amount_paid__sum'] or Decimal('0.00')
        inv.balance_amount = inv.total_amount - inv.paid_amount
...
def invoice_delete(request, pk):
    """Удаление счёта (только POST)."""
    if request.method != 'POST':
        return HttpResponse(status=405)
    invoice = get_object_or_404(Invoice, pk=pk)
    invoice.delete()
    messages.success(request, f"Счёт №{invoice.invoice_number} удалён.")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if payment.amount_paid > balance:
                form.add_error('amount_paid', f"Сумма платежа превышает остаток задолженности в размере ₽{balance}.")
            else:
                payment.save()
                ...
                messages.success(request, f"Записан платёж на сумму ₽{payment.amount_paid} по счёту №{invoice.invoice_number}")
...
def download_invoice_pdf(request, pk):
    ...
    if pdf:
        filename = f"Счет_{invoice.invoice_number}.pdf"
        response = HttpResponse(pdf, content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="{filename}"'
        return response
    return HttpResponse("Ошибка генерации PDF-счёта", status=500)